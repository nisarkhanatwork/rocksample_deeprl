...
from rock_base import BaseSimulator
from rock_basic import manhattan
...
#    ACTIONS_SAMPLE = 4
    ...
    def reset(self):
        ...
        RockSample.my_quality = deepcopy(quality)
    ...
    def simulate(state, action):
        ...
        for i in range(len(rocks)):
            if(quality[i] == 0):
                empty_tuples.append((rocks[i].x, rocks[i].y))
            else:
                rock_tuples.append((rocks[i].x, rocks[i].y))
        return State(rover, rocks, quality), reward, terminal, rock_tuples, empty_tuples
    ...
    def state_to_tensor(state):
        ...
        return t.constant_pad_nd(tensor, (1, 1, 1, 1), value=2)[0].numpy()
